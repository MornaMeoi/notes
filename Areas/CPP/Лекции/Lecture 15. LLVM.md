...
```
И снова возвращается к Codegen.hpp.
#### Обсуждение
...
Далее лектор на примере следующего кода на ParaCL
```pcl
a = ?;
b = a + 3;
c = 10 - a - b;
...
рассматривает
```bash
./ParaCL ../tests-pcl/simplest/addsub.pcl
```
и комментирует, что получившийся LLVM IR ужасен с точки зрения оптимизаций. Затем он делает
```bash
clang++ addsub.pcl.ll -O2 -S -masm=intel -o addsub.pcl.s
```
и отмечает, что ассемблер получился идеальным. Далее лектор отмечает, что это получилось за счет оптимизаций и предлагает их посмотреть через
```bash
clang++ addsub.pcl.ll -O2 -S -masm=intel -o addsub.pcl.s -mllvm --print-after-all >& all.log
```
...
....
```
Далее лектор на примере тех же чисел collatz показывает, что благодаря LLVM ParaCL работает почти с такой же скоростью, как и C++. На 10-ти миллионах ParaCL отработал за 6.859с, а C++ за 2.797с.
...
• Допустим, нам надо поддержать программу на парасиле вроде такой:
```pcl
foo = func(a, b) {
	// много кода
	a = 1;
