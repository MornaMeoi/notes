...
• Итератор может быть невалиден по ряду причин:
	...
	• Он подвис, т.е. ссылается на объект с истёкшим сроком жизни.
...
• Уже рассматривались ранее и иногда в них не ничего плохого.
```cpp
std::istream_iterator<string> beg(ifstream("in.txt")), end;
```
...
```
• Здесь сингулярный итератор невалиден и случается UB.
...
• Итератор может быть инвалидирован операциями над контейнерами.
```cpp
...
```
• Здесь итератор может быть <span style="color: red;">инвалидирован</span>, и куда он указывает не определено. К сожалению, если итератор реализован как указатель, он будет куда-то указывать и ошибка может быть "тихой".
...
```
• Здесь итератор может быть инвалидирован, если ёмкость вектора закончилась и случился `realloc`. А может и не быть.
...
• Удаление
	...
	• deque - инвалидируются все итераторы при удалении из середины. При удалении из начала или конфа, все итераторы сохраняются (кроме итератора на удаляемый).
...
isstream_iterator<string> beg(file), fin;
...
• Абстрагирует особенности Windows и Unix-based систем.
