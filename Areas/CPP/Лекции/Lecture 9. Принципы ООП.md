...
#### Пример плохого проектирование (ISP)
```cpp
struct IWorker {
	virtual void work() = 0;
	virtual void eat() = 0;
...
```
• Здесь менеджер зависит от интерфейса eat. В итоге, его должны реализовывать роботы.
...
• Объективные критерии качества есть, но они очевидно не о том.
	...
	• искуственные критерии вроде цикломатической сложности и т.д. (увы, но все эти требования может легко выполнить чудовищная адская индусская лапша)
• Субъективные критерии ("когда я лично назову код хорошим")
	• читаемость
	• расширяемость
	• разумный выбор алгоритмов и абстракций
• Любой человек защищается. Главное свойство плохого кода: <span style="color: red;">его написал не я</span>.
...
## Паттерны проектирования
 Лектор сказал, что нет смысла обсуждать эту объемную тему так кратко и предложил почитать литературу, которая предложена в конце темы. Тем не менее, далее законспектированы слайды, которые он быстро пролистал:
...
• **Фабричный метод** - статический метод, выполняющий функции "виртуального деструктора".
...
• В инфраструктуре LLVM мы хотим работать с любыми, даже с самыми причудливыми ассемблерами.
```cpp
// we want ADD R0, R1, 1
...
• **Цепочка возможностей** - серия возможно объектов-обработчиков команды (например, обработка и перевыброс исключений).
...
• **Фасад** - облегчённый интерфейс для сложного контекста.
...
```
• Домашняя наработка: написать такие классы как IntVal, DIVObs и MODObs, которые бы позволили наблюдение.
#### Литература
...
#### Бонус: антипаттерны
• **Детонатор** - паттерн, который ждёт в вашем коде и готов в любой момент разнести всё к чертям.
	• Хороший пример: отсутствие проверки на нулевой указатель.
• **Бригада** - контейнерный класс для кривого и косого кода, методы в котором были отвергнуты разработчиками остальных классов.
	• Вместе они - БРИГАДА
