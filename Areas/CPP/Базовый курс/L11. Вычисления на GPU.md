...
> [!abstract]- Оглавление
> - [[#Логическая модель OpenCL|Логическая модель OpenCL]]
>     - [[#Логическая модель OpenCL#Гетерогенные системы|Гетерогенные системы]]
>     - [[#Логическая модель OpenCL#Модель вычислений OpenCL|Модель вычислений OpenCL]]
>     - [[#Логическая модель OpenCL#Запросить платформы и устройства|Запросить платформы и устройства]]
>     - [[#Логическая модель OpenCL#Модель владения OpenCL|Модель владения OpenCL]]
>     - [[#Логическая модель OpenCL#Подсчёт ссылок|Подсчёт ссылок]]
>     - [[#Логическая модель OpenCL#Создать контекст, очередь, буфер|Создать контекст, очередь, буфер]]
>     - [[#Логическая модель OpenCL#Пересылка буфера на OpenCL C++|Пересылка буфера на OpenCL C++]]
>     - Модель вычислений OpenCL
>     - [[#Логическая модель OpenCL#Итерационное пространство задачи|Итерационное пространство задачи]]
>     - [[#Логическая модель OpenCL#Одна инструкция - много потоков|Одна инструкция - много потоков]]
>     - Итерационное пространство задачи
>     - [[#Логическая модель OpenCL#Идея глобальной памяти|Идея глобальной памяти]]
>     - [[#Логическая модель OpenCL#Где мы в глобальной памяти|Где мы в глобальной памяти]]
>     - [[#Логическая модель OpenCL#C++ API: kernel functor|C++ API: kernel functor]]
>     - [[#Логическая модель OpenCL#Информация о выполнении|Информация о выполнении]]
>     - [[#Логическая модель OpenCL#Взаимозависимость кернелов|Взаимозависимость кернелов]]
> - [[#Память и синхронизация|Память и синхронизация]]
>     - [[#Память и синхронизация#Обсуждение: а что для матриц?|Обсуждение: а что для матриц?]]
>     - [[#Память и синхронизация#Case study: перемножение матриц|Case study: перемножение матриц]]
>     - [[#Память и синхронизация#Локальная память в OpenCL|Локальная память в OpenCL]]
>     - [[#Память и синхронизация#Локальная память: управление с хоста|Локальная память: управление с хоста]]
>     - [[#Память и синхронизация#Локальная память внутри ядра|Локальная память внутри ядра]]
>     - [[#Память и синхронизация#Приватная память в OpenCL|Приватная память в OpenCL]]
>     - [[#Память и синхронизация#Summary: память|Summary: память]]
>     - [[#Память и синхронизация#Case study: улучшаем матрицы|Case study: улучшаем матрицы]]
>     - [[#Память и синхронизация#Идея барьеров|Идея барьеров]]
>     - Case study: улучшаем матрицы
>     - [[#Память и синхронизация#Демонстрация|Демонстрация]]
>     - [[#Память и синхронизация#Гистограмма|Гистограмма]]
>     - [[#Память и синхронизация#Идея: делим ответственность|Идея: делим ответственность]]
>     - [[#Память и синхронизация#Сложение должно быть атомарным|Сложение должно быть атомарным]]
>     - [[#Память и синхронизация#Идея: подключаем локальную память|Идея: подключаем локальную память]]
>     - [[#Память и синхронизация#Демонстрация и обсуждение|Демонстрация и обсуждение]]
> - [[#OpenCL C++ API|OpenCL C++ API]]
>     - [[#OpenCL C++ API#Первый шаг: reference handler|Первый шаг: reference handler]]
>     - [[#OpenCL C++ API#Идея специализации|Идея специализации]]
>     - [[#OpenCL C++ API#Собираем reference handler|Собираем reference handler]]
>     - [[#OpenCL C++ API#Второй шаг: wrapper|Второй шаг: wrapper]]
>     - [[#OpenCL C++ API#Третий шаг: девайс|Третий шаг: девайс]]
>     - [[#OpenCL C++ API#Выход: специальный конструктор|Выход: специальный конструктор]]
>     - [[#OpenCL C++ API#Этюд: получение информации|Этюд: получение информации]]
>     - [[#OpenCL C++ API#Идея: type traits|Идея: type traits]]
>     - Этюд: получение информации
>     - [[#OpenCL C++ API#Иерархия классов|Иерархия классов]]
> - [[#Битоническая сортировка|Битоническая сортировка]]
>     - [[#Битоническая сортировка#Монотонные последовательности|Монотонные последовательности]]
>     - [[#Битоническая сортировка#Bitonic merge|Bitonic merge]]
>     - [[#Битоническая сортировка#HWB: битоническая сортировка|HWB: битоническая сортировка]]
>     - [[#Битоническая сортировка#Литература|Литература]]
...
![[_Meta/attachments/Базовый курс/L11/11.1.png]]
#### Подсчёт ссылок
...
![[_Meta/attachments/Базовый курс/L11/11.2.png]]
#### Итерационное пространство задачи
...
![[_Meta/attachments/Базовый курс/L11/11.3.png]]
...
![[_Meta/attachments/Базовый курс/L11/11.4.png]]
...
![[_Meta/attachments/Базовый курс/L11/11.5.png]]
...
![[_Meta/attachments/Базовый курс/L11/11.6.png]]
...
![[_Meta/attachments/Базовый курс/L11/11.7.png]]
#### Summary: память
• Хостовая память.
...
#### Идея барьеров
![[_Meta/attachments/Базовый курс/L11/11.8.png]]
...
![[_Meta/attachments/Базовый курс/L11/11.9.png]]
...
![[_Meta/attachments/Базовый курс/L11/11.10.png]]
...
```
![[_Meta/attachments/Базовый курс/L11/11.11.png]]
Пример с гита:
```cpp
//-----------------------------------------------------------------------------
//
...
![[_Meta/attachments/Базовый курс/L11/11.12.png]]
#### Bitonic merge
...
![[_Meta/attachments/Базовый курс/L11/11.13.png]]
