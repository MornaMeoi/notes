...
> [!abstract]- Оглавление
> - [[#GPU и OpenGL|GPU и OpenGL]]
>     - [[#GPU и OpenGL#GPU как вычислительная система|GPU как вычислительная система]]
>     - [[#GPU и OpenGL#Давайте отрендерим квадрат|Давайте отрендерим квадрат]]
>     - [[#GPU и OpenGL#Фиксированный конвейер|Фиксированный конвейер]]
>     - [[#GPU и OpenGL#Общение с рантаймом|Общение с рантаймом]]
>     - [[#GPU и OpenGL#Расширения OpenGL: буферы вершин|Расширения OpenGL: буферы вершин]]
>     - [[#GPU и OpenGL#Расширения и версии|Расширения и версии]]
>     - [[#GPU и OpenGL#Нефиксированный конвейер|Нефиксированный конвейер]]
>     - [[#GPU и OpenGL#Вершинные шейдеры|Вершинные шейдеры]]
>     - [[#GPU и OpenGL#Binding points: glBindBuffer|Binding points: glBindBuffer]]
>     - [[#GPU и OpenGL#Что такое "фрагмент"?|Что такое "фрагмент"?]]
>     - [[#GPU и OpenGL#Фрагментные шейдеры|Фрагментные шейдеры]]
>     - [[#GPU и OpenGL#Uniform и varying переменные|Uniform и varying переменные]]
>     - [[#GPU и OpenGL#Компиляция и исполнение шейдеров|Компиляция и исполнение шейдеров]]
>     - [[#GPU и OpenGL#Обсуждение: а где же 3D?|Обсуждение: а где же 3D?]]
> - [[#Логическая модель|Логическая модель]]
>     - [[#Логическая модель#Обсуждение: сцена и отображение|Обсуждение: сцена и отображение]]
> - [[#Шейдер для трансформации|Шейдер для трансформации]]
>     - [[#Шейдер для трансформации#Давайте отрендерим куб|Давайте отрендерим куб]]
>     - [[#Шейдер для трансформации#Первая проблема: culling|Первая проблема: culling]]
>     - [[#Шейдер для трансформации#Вторая проблема: depth|Вторая проблема: depth]]
>     - [[#Шейдер для трансформации#Режимы геометрии: QUADS vs LINES|Режимы геометрии: QUADS vs LINES]]
>     - [[#Шейдер для трансформации#Обсуждение: архитектура|Обсуждение: архитектура]]
>     - [[#Шейдер для трансформации#Что происходит в программе?|Что происходит в программе?]]
>     - [[#Шейдер для трансформации#Обсуждение: что такое вершина?|Обсуждение: что такое вершина?]]
>     - [[#Шейдер для трансформации#Идея сцены и простые вершины|Идея сцены и простые вершины]]
>     - [[#Шейдер для трансформации#Ассиметрия в параметрах шейдера|Ассиметрия в параметрах шейдера]]
>     - [[#Шейдер для трансформации#Uniform buffer objects|Uniform buffer objects]]
> - [[#Vulkan API|Vulkan API]]
>     - [[#Vulkan API#На пути к Vulkan|На пути к Vulkan]]
>     - [[#Vulkan API#Концпептуальаня модель Вулкана|Концпептуальаня модель Вулкана]]
>     - [[#Vulkan API#Основы: Instance, Device, Queue|Основы: Instance, Device, Queue]]
>     - [[#Vulkan API#Рендеринг: swap chain, images, pipeline|Рендеринг: swap chain, images, pipeline]]
>     - [[#Vulkan API#Пасс рендеринга|Пасс рендеринга]]
>     - [[#Vulkan API#Фиксированный конвейер|Фиксированный конвейер]]
>     - [[#Vulkan API#Прекомпиляция шейдеров|Прекомпиляция шейдеров]]
>     - [[#Vulkan API#Буфер команд|Буфер команд]]
>     - [[#Vulkan API#Цикл отображения|Цикл отображения]]
>     - [[#Vulkan API#Проблема синхронизации|Проблема синхронизации]]
>     - [[#Vulkan API#Семафоры для синхронизации|Семафоры для синхронизации]]
>     - [[#Vulkan API#Фенсы для синхронизации|Фенсы для синхронизации]]
>     - [[#Vulkan API#Управление памятью|Управление памятью]]
>     - [[#Vulkan API#Отображение памяти|Отображение памяти]]
>     - [[#Vulkan API#Command buffer спешит на помощь|Command buffer спешит на помощь]]
>     - [[#Vulkan API#Демо|Демо]]
> - [[#Физическая и объектная модель|Физическая и объектная модель]]
>     - [[#Физическая и объектная модель#Что происходит в программе на Vulkan?|Что происходит в программе на Vulkan?]]
>     - [[#Физическая и объектная модель#Vulkan - это объектная модель|Vulkan - это объектная модель]]
>     - [[#Физическая и объектная модель#Но она немного C-style|Но она немного C-style]]
>     - [[#Физическая и объектная модель#Vulkan-Hpp: C++ API|Vulkan-Hpp: C++ API]]
>     - [[#Физическая и объектная модель#Безопасные флаги|Безопасные флаги]]
>     - [[#Физическая и объектная модель#Доступ к нижележащему типу|Доступ к нижележащему типу]]
>     - [[#Физическая и объектная модель#Размер кода существенно улучшается|Размер кода существенно улучшается]]
>     - [[#Физическая и объектная модель#Unique pointers/resources|Unique pointers/resources]]
>     - [[#Физическая и объектная модель#Литература|Литература]]
...
glfwPollEvents();
```
![[_Meta/attachments/Базовый курс/L10/10.1.png]]
Пример с гита:
```cpp
//-------------------------------------------------------------------------------
//
...
```
![[_Meta/attachments/Базовый курс/L10/10.2.png]]
...
![[_Meta/attachments/Базовый курс/L10/10.3.png]]
#### Фрагментные шейдеры
...
```
![[_Meta/attachments/Базовый курс/L10/10.4.gif]]
...
![[_Meta/attachments/Базовый курс/L10/10.5.png]]
...
![[_Meta/attachments/Базовый курс/L10/10.6.gif]]
...
```
![[_Meta/attachments/Базовый курс/L10/10.7.png]]
• Демонстрирует face culling.
![[_Meta/attachments/Базовый курс/L10/10.8.png]]
...
#### Пасс рендеринга
![[_Meta/attachments/Базовый курс/L10/10.11.svg]]
#### Фиксированный конвейер
...
![[_Meta/attachments/Базовый курс/L10/10.12.svg]]
#### Цикл отображения
...
![[_Meta/attachments/Базовый курс/L10/10.13.svg]]
...
![[_Meta/attachments/Базовый курс/L10/10.14.svg]]
...
```
![[_Meta/attachments/Базовый курс/L10/10.15.gif]]
...
![[_Meta/attachments/Базовый курс/L10/10.16.png]]
